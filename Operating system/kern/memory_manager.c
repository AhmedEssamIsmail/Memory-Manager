...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 * address = (uint32 *)kmalloc(PAGE_SIZE);
	memset(address,0,PAGE_SIZE);
	uint32 phys_address = kheap_physical_address((uint32)address);
	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(phys_address , PERM_WRITEABLE |PERM_PRESENT |PERM_USER);
	tlbflush();
	return (void*)address;
}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	struct WorkingSetElement WS_copy[env_page_ws_get_size(e)];
	int index = 0;
	for(int i=0;i<WS_size;i++)
	{
		uint32 VA = env_page_ws_get_virtual_address(e,i);
		if(!env_page_ws_is_entry_empty(e,i) && (VA >= virtual_address && VA < virtual_address + size))
			unmap_frame(e->env_page_directory,(void*)VA);
		else if(!env_page_ws_is_entry_empty(e,i) )
		{
			WS_copy[index] = e->ptr_pageWorkingSet[i];
			index++;
		}
		env_page_ws_clear_entry(e,i);
	}
	for(int i=0;i<index;i++)
		e->ptr_pageWorkingSet[i] = WS_copy[i];

	e->page_last_WS_index = index % WS_size;
...
{
	if(entry_index < 0 || entry_index >= (e->page_WS_max_size))
		cprintf("bydrab hena %s , %d \n",e->prog_name,entry_index);
	assert(entry_index >= 0 && entry_index < (e->page_WS_max_size));
