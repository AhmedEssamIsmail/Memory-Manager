...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	...
	if(env_page_ws_get_size(curenv) >= curenv->page_WS_max_size)
	{
		...
		uint32 * ptr;
		struct Frame_Info * victim_va_info = get_frame_info(curenv->env_page_directory,(void*)victim_va,&ptr);
		uint32 modified = ptr[PTX(victim_va)] & PERM_MODIFIED;
		if(modified)
		{
			ret = pf_update_env_page(curenv, (void*)victim_va,victim_va_info);
			if(ret == E_PAGE_NOT_EXIST_IN_PF)
				panic("ERROR: Page doesn't exist in the page file");
		}
		unmap_frame(curenv->env_page_directory,(void*)victim_va);
		env_page_ws_clear_entry(curenv,curenv->page_last_WS_index);
	}
		struct Frame_Info * fault_va_info ;
		ret = allocate_frame(&fault_va_info);
		if(ret != E_NO_MEM)
			map_frame(curenv->env_page_directory,fault_va_info,(void*)fault_va,PERM_USER|PERM_WRITEABLE|PERM_PRESENT);
		else
			panic("ERROR: No free frames for the new page");
