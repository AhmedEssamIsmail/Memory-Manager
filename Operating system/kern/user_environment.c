		...
		, unsigned int phys_user_page_directory)
{
...
#if USE_KHEAP == 1
	{
		...
		{
			//e->env_page_directory[PDX(sva)] = ptr_page_directory[PDX(sva)];
...
void env_free(struct Env *e)
{
	...
	{
		if((e->env_page_directory[i] & (PERM_PRESENT|PERM_USED)) == (PERM_PRESENT|PERM_USED))
		{
			void* ptr = (void*)kheap_virtual_address(e->env_page_directory[i]);
			kfree(ptr);
		}
	}
