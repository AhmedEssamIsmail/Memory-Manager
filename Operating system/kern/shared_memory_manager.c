...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	for(int i =0;i<pages;i++)
	{
		...
		allocate_frame(&info);
		map_frame(myenv->env_page_directory,info,virtual_address,PERM_USER | PERM_WRITEABLE);
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	uint32 pages = ROUNDUP(shares[index].size,PAGE_SIZE) / PAGE_SIZE;
	int perms = PERM_USER;
	...
	for(int i=0;i<pages;i++)
	{
		struct Frame_Info * info = get_frame_from_storage(shares[index].framesStorage,i);
		map_frame(myenv->env_page_directory,info,virtual_address,perms);
		virtual_address+=PAGE_SIZE;
	}
...
int freeSharedObject(int32 sharedObjectID, void *startVA)
{
	...
	if(get_share_object_ID(shares[sharedObjectID].ownerID,shares[sharedObjectID].name) == E_SHARED_MEM_NOT_EXISTS)
		return E_SHARED_MEM_NOT_EXISTS;

	uint32 size = shares[sharedObjectID].size;
	uint32 pages = ROUNDUP(size,PAGE_SIZE)/PAGE_SIZE;
	uint32 VA = (uint32)startVA;

	for(int i=0;i<pages;i++,VA += PAGE_SIZE)
		unmap_frame(myenv->env_page_directory,(void*)VA);

	VA=(uint32)startVA;
	for(int i=0;i<pages;i++,VA+=PAGE_SIZE)
	{
		uint32 * ptr = NULL;
		get_page_table(myenv->env_page_directory,(void*)VA,&ptr);
		if(ptr != NULL)
		{
			int empty = 1;
	...
	if(!--shares[sharedObjectID].references)
